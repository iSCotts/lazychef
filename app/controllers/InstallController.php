...
class InstallController extends Controller {
	...
	public function publishAndMigrate()
	{
		$artisan = Artisan::call(
			...
			array(
				'--env' => App::environment(),
				'--database' => 'lazychef',
				'--package' => 'lazychef/core'
			)
		);

		Artisan::call(
			'asset:publish',
			array('package' => 'lazychef/core')
		);
	 ...
	 * @return
	 */
	protected function setLazychefConfig($title, $theme, $per_page)
	{
		$path = $this->getConfigFile('lazychef.php');
		$content = str_replace(
	...
	{
		if (file_exists(app_path().'/config/packages/lazychef/core/'.App::environment().'/'.$file))
		{
			return app_path().'/config/packages/lazychef/core/'.App::environment().'/'.$file;
		}

		return app_path().'/config/packages/lazychef/core/'.$file;
	}
}
